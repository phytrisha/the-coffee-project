...
struct Order: Codable, Identifiable {
    ...
    var userId: String
    var shopId: String
    var items: [Drink] // Array, since in the future, more items could be included
    var totalAmount: Float
    var orderDate: Date
    var status: String

    // Initializer to create a new order
    init(userId: String, shopId: String, item: Drink) {
        self.userId = userId
        self.shopId = shopId
        self.items = [item] // Always store a single item in the array for this case
        self.totalAmount = item.price // Now only works for a single item
        self.orderDate = Date()
